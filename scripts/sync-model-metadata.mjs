 ...
 * 仍由 Biny 的 ProviderDefinition 与用户配置决定。地区和订阅路径使用各自的模型事实；
 * 只有事实一致的别名复用同一份 metadata，订阅不会继承普通 API 的完整目录。
 */
export const PROVIDERS = {
  deepseek: { source: "deepseek", catalog: true, endpoints: ["https://api.deepseek.com", "https://api.deepseek.com/v1"] },
  openai: { source: "openai", catalog: true, endpoints: ["https://api.openai.com/v1"] },
  anthropic: { source: "anthropic", catalog: true, endpoints: ["https://api.anthropic.com", "https://api.anthropic.com/v1"] },
  "claude-subscription": { source: "anthropic", aliasOf: "anthropic", catalog: false },
  "openai-codex": { source: "openai", aliasOf: "openai", catalog: false, endpoints: ["https://chatgpt.com/backend-api/codex"] },
  gemini: { source: "google", catalog: true, endpoints: ["https://generativelanguage.googleapis.com/v1beta/openai", "https://generativelanguage.googleapis.com/v1beta"] },
  kimi: { source: "moonshotai", catalog: true, endpoints: ["https://api.moonshot.ai/v1"] },
  "kimi-cn": { source: "moonshotai-cn", catalog: false, endpoints: ["https://api.moonshot.cn/v1"] },
  "kimi-coding": {
    source: "kimi-for-coding", catalog: false,
    endpoints: ["https://api.kimi.com/coding", "https://api.kimi.com/coding/v1"],
    // 官方模型配置已将标准编程模型升级到 1M；models.dev 暂仍保留旧版 256K。
    // https://www.kimi.com/code/docs/kimi-code/models.html（2026-09-12 核对）
    metadataOverrides: { "kimi-for-coding": { contextWindow: 1_048_576 } }
  },
  qwen: { source: "alibaba-cn", catalog: true, endpoints: ["https://dashscope.aliyuncs.com/compatible-mode/v1"] },
  "qwen-international": { source: "alibaba", catalog: false, endpoints: ["https://dashscope-intl.aliyuncs.com/compatible-mode/v1"] },
  zai: { source: "zai", catalog: true, endpoints: ["https://api.z.ai/api/paas/v4"] },
  "zai-coding-plan": { source: "zai-coding-plan", catalog: false, endpoints: ["https://api.z.ai/api/coding/paas/v4"] },
  zhipu: { source: "zhipuai", catalog: false, endpoints: ["https://open.bigmodel.cn/api/paas/v4"] },
  "zhipu-coding-plan": { source: "zhipuai-coding-plan", catalog: false, endpoints: ["https://open.bigmodel.cn/api/coding/paas/v4", "https://open.bigmodel.cn/api/anthropic", "https://open.bigmodel.cn/api/v1"] },
  openrouter: { source: "openrouter", catalog: true, endpoints: ["https://openrouter.ai/api/v1"] }
};
      ...
      const models = Object.fromEntries(
        Object.entries(provider.models)
          ...
          .flatMap(([modelId, model]) => {
            ...
            return metadata ? [[modelId, { ...metadata, ...definition.metadataOverrides?.[modelId] }]] : [];
          })
      );
...
export function buildGeneratedModule(generated, aliases, catalogProviders) {
  // 端点白名单由 Biny 维护，只用于选择模型事实，不从上游导入可执行的请求路由。
  const endpoints = Object.fromEntries(Object.entries(PROVIDERS)
    .flatMap(([provider, definition]) => (definition.endpoints ?? []).map((endpoint) => [endpoint, provider])));
  return [
    ...
    ";",
    "",
    "export const GENERATED_MODELS_DEV_METADATA_ENDPOINTS: Record<string, string> =",
