 ...
 *
 * 补分析直接使用统一工具模型；无可用模型时只渲染已分析部分并说明原因。
 */
import { z } from "zod";
...
export function createActivityReportTool(deps: ActivityReportToolDeps): Tool<ActivityReportArgs, string> {
  return {
    ...
    description: [
      ...
      "Analyzing sessions that have no result yet uses the configured tool model. Without an available model, the report covers only stored analyses and explains what is missing."
    ].join(" "),
      ...
      return {
        ...
        async execute({ signal }) {
          ...
          try {
            ...
            // 缓存必须同时受原始事件、分析输入和模型影响；否则今天新增活动或模型切换后，
