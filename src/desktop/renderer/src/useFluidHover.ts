/**
 * Fluid Functionalism 的 useFluidHover：追踪容器内离指针最近的条目，
 * 供 FluidHoverHighlight 在条目之间做弹簧位移。原样移植自上游 registry，
 * 仅去掉 Next.js 的 "use client" 指令；注册必须走 useRegisterFluidHoverItem。
 */
...
export function useFluidHover<T extends HTMLElement>(
  ...
  const scheduleMeasurement = useCallback(
    (attemptsLeft: number) => {
      // 局部递归而不是回调自引用：useCallback 的常量在自己的初始化语句里还不可用。
